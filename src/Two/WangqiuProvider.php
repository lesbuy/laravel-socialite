...
class WangqiuProvider extends AbstractProvider implements ProviderInterface
{
    ...
    protected function mapUserToObject(array $user)
    {
        return (new User)->setRaw($user)->map([
            'id' => $user['wxid'],
			'type' => Config::get('const.USERTYPE_WX_WANGQIU'),
			'nickname' => $user['name'], 
			'name' => $user['name'],
			'email' => null, 
			'avatar' => null,
	...
	protected function decrypt($cookie) {
		...
		if ($arr[3] != Config::get('const.USERTYPE_WX_WANGQIU')) return false;
